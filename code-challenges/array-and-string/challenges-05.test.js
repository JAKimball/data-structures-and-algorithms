...
const listFoods = (recipe) => {
  ...
  recipe.ingredients.forEach(item => {
    result.push(item.replace(/(\S+\s+){2}/, ''));
  });
  return result;
};
...
const removeLastCharacters = (str, numberOfCharacters) => {
  let len = str.length;
  return str.slice(0, Math.max(0,Math.min(len, len-numberOfCharacters)));
};
...
const totalSumCSV = (str) => {
  let total = 0;
  str.split(',').forEach((item) => { total += parseInt(item); });
  return total;
};
...
const extractVowels = (str) => {
  ...
  return [str.replace(regex, ''), str.match(regex).sort((a, b) => a > b ? 1 : -1).join('')];
};
