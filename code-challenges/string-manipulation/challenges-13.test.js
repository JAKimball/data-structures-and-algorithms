...
const standardizePhoneNumbers = (arr) =>
  arr.map(str => `${str.substring(1, 4)}${str.substring(6, 9)}${str.substring(10, 14)}`);
...
const allHappy = (arr) => arr.reduce((acc, cur) => acc && cur.includes(':)'), true);
...
const findEvery = (arr, target) =>
  arr.reduce((acc, cur) => acc && cur.includes(target), true);
...
const unenrollBrook = (arr) => arr
  .map(arr => arr.filter(str => !str.includes('Brook')));
...
const sortByDay = (arr) =>
  daysOfWeek.map(dayStr => arr.filter(event => event.includes(dayStr)));
...
const characterByIndex = (arr) =>
  arr.map((val, idx) => val.charAt(idx));
