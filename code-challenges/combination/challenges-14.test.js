...
const toTitleCase = (arr) => arr.map(str => {
...
let massOf = (name) => parseInt(starWarsData
  .filter(char => char.name.toLowerCase() === name.toLowerCase())[0]
  .mass);

let biggerThanLuke = (arr) =>
  arr
    .filter(char => parseInt(char.mass) > massOf('Luke Skywalker'))
    .map(obj => obj.name)
...
const checkAt = (cell1, cell2, cell3) =>
  cell1 === '' ? false :
    cell1 !== cell2 ? false :
      cell1 !== cell3 ? false : true;
...
const detectTicTacToeWin = (board) => {
  ...
  // call checkAt until win is found!
    ...
    // test rows
    checkAt(board[0][0], board[0][1], board[0][2]) ||
    checkAt(board[1][0], board[1][1], board[1][2]) ||
    checkAt(board[2][0], board[2][1], board[2][2]) ||
    // test columns
    checkAt(board[0][0], board[1][0], board[2][0]) ||
    checkAt(board[0][1], board[1][1], board[2][1]) ||
    checkAt(board[0][2], board[1][2], board[2][2]) ||
    // test diagonals
    checkAt(board[0][0], board[1][1], board[2][2]) ||
    checkAt(board[2][0], board[1][1], board[0][2])
  );
};
