...
const hasChildrenEntries = (arr, character) => {
  ...
  arr.forEach(char => {
    if (char.name === character) {
      Object.entries(char).forEach(entry => {
        if (entry[0] === 'children') {
          result = entry[1].length !== 0;
        }
      });
    }
  });
...
const totalCharacters = (arr) => {
  let result = 0;
  arr.forEach(char => {
    result += 1 + (char.spouse ? 1 : 0) + char.children.length;
  });
  return result;
};
...
const houseSize = (arr) => {
  ...
  arr.forEach(char => {
    let house = {};
    house.house = char.house;
    house.members = 1 + (char.spouse ? 1 : 0) + char.children.length;
    sizes.push(house);
  });
  return sizes;
};
...
const houseSurvivors = (arr) => {
  const survivors = [];
  const dead = {};
  deceasedSpouses.forEach(spouse => { dead[spouse] = true; });
  arr.forEach(char => {
    let house = {};
    house.house = char.house;
    house.members = 1 + (char.spouse ? 1 : 0) + char.children.length;
    house.members -= ((char.spouse in dead) ? 1 : 0);
