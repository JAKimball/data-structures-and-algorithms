...
const count = (target, input) =>
  input.map(subArr => subArr
    .reduce((acc, cur) => cur === target ? ++acc : acc, 0))
...
const divisibleByFiveTwoToThePower = (input) =>
  input.map(subArr => subArr.filter(n => !isNaN(n) && (typeof n === 'number') && (n % 5 === 0))
    .map(n => 2 ** n));
...
const findShortest = (data) =>
  data
    .reduce((acc, cur) => parseFloat(acc.height) > parseFloat(cur.height) ? cur : acc)
    .name;
